...
% Author setting
\usepackage{authblk}
\newcommand{\orcid}[1]{\href{https://orcid.org/#1}{\includegraphics[height=1em]{orcid.png}}}

\title{\Large\textbf{Candidates in American General Elections}}

\author[1]{\normalsize Jeremiah Cha\,\orcid{0000-0003-3199-3386}}
\author[2]{\normalsize Shiro Kuriwaki\,\orcid{0000-0002-5687-2647}}
\author[1]{\normalsize James M. Snyder, Jr.\,\orcid{0000-0002-1386-2844}}
...
\date{\vspace{-0.2in} \normalsize Version 4.0 (February 2026)}
...
Cha, Jeremiah, Shiro Kuriwaki, and James M. Snyder, Jr., 2026, ``Candidates in American General Elections'', Harvard Dataverse. \textsc{doi}: \href{https://doi.org/10.7910/DVN/DGDRDT}{\texttt{10.7910/DVN/DGDRDT}}.
\end{minipage}
\end{center}
\smallskip

Between 2006 and 2024, there were approximately 5,000 races for Congress and Governor in the general election, in which \input{Ns/all_cands}\unskip \ Democratic and Republican candidates ran for office.
...
We provide a candidate-level dataset that is comprehensive for Congressional and gubernatorial contests between 2006 and 2024, sourced from the official House clerk website but with standardized names for each unique candidate. We also append Presidential statewide election results from MEDSL's compilation. We also record party, incumbency, vote totals, and election results. For example:
...
Presidential vote counts at the state level are taken from the data compiled by the MIT Election Data Science Lab (\url{https://doi.org/10.7910/DVN/42MVDX}), which helpfully standardizes party names of Presidential candidates, even for minor parties.
...
This dataset excludes candidates who receive less than ten votes in their general election and those who list Blank or Miscellaneous as their party.
...
To identify a single election, group by \texttt{year}, \texttt{state}, \texttt{office}, \texttt{dist}, and \texttt{type} (General vs. Special).  To isolate the set of candidates who won a seat in an election cycle, group by \texttt{year}, \texttt{state}, \texttt{office}, and \texttt{type}, then subset by \texttt{won}.
...
In very rare instances, a candidate for a major party runs for office in two different states. For example, Mitt Romney ran for Governor of Massachusetts, President, and U.S. Senator from Utah. For these candidates, group by \texttt{name\_snyder}.
% \clearpage
...
The district the candidate is running for. U.S. House races in at-large districts are numbered 1 instead of 0 or ``-AL''.
 ...
 For U.S. Senate candidates, the district variable denotes the class of the particular Senate seat (e.g. Dianne Feinstein had a \texttt{dist} value of 1 since her seat is part of Senate Class I).
...
specifics of these coding rules are important for tracking over time because
...
these cases, the Snyder formalization of identifying people by their full
...
{different} names. For example, suppose a John Smith ran for Congress
in 1980 in Ohio and then a \emph{separate, unrelated} John Smith ran
again in 2020 in Ohio. To distinguish the two different people, we
...
The MIT Election Data Science Lab (\url{https://dataverse.harvard.edu/dataverse/medsl\_election\_returns}) produces similar election result data. Their data goes back as far as 1976. The data does not include incumbency status, and the names are taken as officially reported rather than standardized to be consistent across years.

% \section{Extensions: Race, Gender, and Primary Elections}

Our dataset does not contain information about the gender or race of the candidates. We hope to collect that data and combine other sources of data in the future.  In the meantime there are several related data sources users can rely on. First, numerous years in the CES have data available on candidate race and gender for interested researchers. Please see the table below for further information regarding the availability of such information by year and where it can be located.
...
If you find any errors or ambiguities, please do not hesitate to contact us (e.g. at \url{https://orcid.org/0000-0002-5687-2647}). The preferred method is to open a ``Issue'' at \url{https://github.com/kuriwaki/cces_candidates/issues}, which we monitor. Otherwise, please email us at any of our emails.
...
\begin{itemize}
\item Dataverse v4.0: Added 2024 general elections (2026-02-14). A few corrections of other issues submitted during this time, e.g. 43, 45.
...
Our thanks to Daniel Moskowitz and Luca Bellodi for submitting issues.
