...
\title{\large\textbf{Standardized Candidate Information and Election Results:\\U.S. Congressional and Governor Elections}}
...
Between 2006-2021 alone, there were \input{Ns/all_dists}\unskip \ races for Congress and Governor in the general election, in which \input{Ns/all_cands}\unskip \ Democratic and Republican candidates ran for office.  Here we provide a {candidate level} dataset that includes new data, such as incumbency, general election vote, party, and the election result.  
While there exist several datasets that compile these election results, few standardized them such that (1) the same candidate is named the same way across years and offices, (2) the incumbency status of the candidate is known, and (3) elections of different offices and years are put together into a single file.

% \section{Examples, Unique Identifiers, and Counts}

Our dataset contains one row per candidate in each election they ran in:
