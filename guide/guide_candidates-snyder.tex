...
%\jccommeent{We should extend this to 2020 and recalculate the race total}
Between 2006-2018 alone, there were \input{Ns/all_dists}\unskip \ races for Congress and Governor in the general election, in which \input{Ns/all_cands}\unskip \ Democratic and Republican candidates ran for office.  Here we provide a \textbf{candidate level} dataset that includes new data, such as incumbency, general election vote, party, and the election result, from historical candidate level datasets compiled by James M. Snyder. There is one row for every candidate in each election they ran in. 
...
%\jccomment{Are we using this for the Snyder Candidate Data release?}
