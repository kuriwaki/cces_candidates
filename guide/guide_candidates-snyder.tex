...
\title{\large\textbf{Standardized Candidate Information and Results in U.S. Congressional and Governor Elections}}
...
The dataset contains one row per candidate in each election they ran in, as shown in Table \ref{tab:cdataexample}.
...
As shown, we create a comprehensive record of the candidates and their elections results. We also collect their incumbency status, and used the same standardized name format so the same candidate running in different years and different offices are uniquely identified.
