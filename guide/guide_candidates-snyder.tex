...
Between 2006 and 2021 alone, there were \input{Ns/all_dists}\unskip \ races for Congress and Governor in the general election, in which \input{Ns/all_cands}\unskip \ Democratic and Republican candidates ran for office.  
...
We provide a candidate-level dataset that is comprehensive for the years 2006 to 2020 with standardized names so candidates can be uniquely identified across years and offices. We also include variables for party, incumbency status, general election vote totals, and election results. For example:
